...
def add_pet():
    ...
    if form.validate_on_submit():
        pet = Pet(nome=form.nome.data,
                  ...
                  foto_url=form.foto_url.data if form.foto_url.data else None,
...
def edit_pet(pet_id):
    ...
    if form.validate_on_submit():
        ...
        pet.foto_url = form.foto_url.data if form.foto_url.data else pet.foto_url # Mantém a URL antiga se o campo vier vazio
...
def delete_pet(pet_id):
    ...
    if pet.adocao and pet.adocao.status != "Cancelada":
        flash("Não é possível remover um pet com um processo de adoção ativo ou confirmado.")
        return redirect(url_for("pets.list_pets"))
    try:
        db.session.delete(pet)
        db.session.commit()
        flash(f"Pet {pet.nome} removido com sucesso.")
        return redirect(url_for("pets.list_pets"))
    except Exception as e:
        db.session.rollback()
        flash(f"Erro ao remover o pet: {e}")
        return redirect(url_for("pets.list_pets"))
